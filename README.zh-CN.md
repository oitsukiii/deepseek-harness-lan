...
> **🎯 目标版本：dsh `0.1.1-rc.2`（commit `b150a55`）** —— 补丁基于该版本编写并验证，其他版本可能失效（详见 [🧩 兼容性](#-兼容性)）。
...
- [✨ 特性](#-特性)
- [🚀 快速开始](#-快速开始)
- [🔀 备选方案：不改源码（SSH 隧道）](#-备选方案不改源码ssh-隧道)
- [🧠 技术原理](#-技术原理)
- [🔒 安全须知（务必阅读）](#-安全须知务必阅读)
- [🧩 兼容性](#-兼容性)
- [🗺️ 版本兼容矩阵](#️-版本兼容矩阵)
...
- 🧪 **已在 dsh 0.1.1-rc.2 验证**：页面加载、API 调用、特权接口全通过
...
>
> 当前 master 分支即目标版本 `0.1.1-rc.2`（commit `b150a55`）。若日后上游已前进（`apply.sh` 会提示版本不匹配），先固定版本再继续：
>
> ```bash
...
**本补丁针对 dsh `0.1.1-rc.2`（commit `b150a55`）编写并验证。**

> ⚠️ **上游说明（0.1.1-rc.2）**：该版本本身存在已知构建问题——`tsc`/`tsdown` 无法解析 `@deepseek-ai/dsh-api-remotes` workspace 包（缺少 `*/remote` 子路径模块）。已在**未打任何补丁的干净上游 checkout** 上验证，**与本补丁无关**。本补丁可干净应用，且 4 个被修改的文件全部通过类型检查（`tsc -b tsconfig.client.json`）；完整构建验证需等上游修复。上一个目标版本（0.1.0-rc.7）构建正常。

- 验证链路：补丁可干净应用（`git apply --check`）→ 4 个被改文件全部通过 `tsc` 类型检查 → Web UI 页面加载 → `/api` 接口调用 → 特权接口（`settings.describe` 等）全通过。
...
| `0.1.0-rc.7`（[release](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.0-rc.7)） | 2026-08-17 | ❌ — 用[旧补丁](https://github.com/oitsukiii/deepseek-harness-lan/commit/511e59ad9735f10c11fd79b1bbd729ef29947305) |
...
| `0.1.1-rc.2`（[release](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.1-rc.2)） | 2026-08-21 | ✅ 当前（[补丁 commit](https://github.com/oitsukiii/deepseek-harness-lan/commit/82a59124813c53bf8dadfc817a2cfdf8b8a3715f)） |
